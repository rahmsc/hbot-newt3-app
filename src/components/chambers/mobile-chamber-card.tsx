...
export function MobileChamberCard({ chamber, onQuickView }: MobileChamberCardProps) {
  ...
  const { name = "", type = "", info = "", id = 0 } = chamber
  ...
  return (
    ...
    >
      <div className="group relative h-[450px] overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-500 to-blue-600 shadow-lg transition-all duration-500 ease-in-out hover:shadow-2xl hover:shadow-emerald-500/20">
        <Image
          src={`${imageUrl}${id}.png`}
          alt={name ?? ""}
          fill
        ...
        <div className="absolute inset-0 flex flex-col justify-between p-6 text-white">
          <div className="space-y-2">
            <h3 className="font-['Raleway'] text-3xl uppercase tracking-wider">{name}</h3>
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-mono text-sm">Type: {type}</span>
              </div>
              {info && (
                <div className="flex items-center space-x-2">
                  <svg
                  ...
                  <span className="font-mono text-sm">Info: {info}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-row items-center justify-center">
            <GlowingButton text="View Details" onClick={() => onQuickView(chamber)} className="w-full max-w-[200px]" />
          </div>
        </div>
      </div>
    </motion.div>
